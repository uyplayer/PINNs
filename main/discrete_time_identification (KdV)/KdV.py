...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, u1, layers, dt, lb, ub, q):
        ...
        self.loss = tf.reduce_sum(tf.square(self.u0_tf - self.U0_pred)) + tf.reduce_sum(
            tf.square(self.u1_tf - self.U1_pred)
        )
    ...
    def predict(self, x_star):
        ...
        U0_star = self.sess.run(
            self.U0_pred,
            {self.x0_tf: x_star, self.dummy_x0_tf: np.ones((x_star.shape[0], self.q))},
        )
        U1_star = self.sess.run(
            self.U1_pred,
            {self.x1_tf: x_star, self.dummy_x1_tf: np.ones((x_star.shape[0], self.q))},
        )
...
if __name__ == "__main__":
    ...
    s2 = r"$u_t + %.3f u u_x + %.7f u_{xxx} = 0$ \\  \hline " % (
        lambda_1_value,
        lambda_2_value,
    )
    s3 = r"Identified PDE (1\% noise) & "
    s4 = r"$u_t + %.3f u u_x + %.7f u_{xxx} = 0$  \\  \hline " % (
